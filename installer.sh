...
surat=$(curl -sS https://raw.githubusercontent.com/errorcode86/akses/main/ipmini | grep -w $ipes | awk '{print $4}')
...
fun_botOnOff() {
  ...
  [[ ! -f /root/multi/bot.conf ]] && {
    echo -e "Phantom86 Bot Panel Installer
        "
    [[ ! -f /root/ResBotAuth ]] && {
    ...
    read bot_user
    [[ -z $bot_user ]] && bot_user="phantom86_bot"
    echo ""
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/errorcode86/bot_panel/main/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
      }
